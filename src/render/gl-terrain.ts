...
void main() {
  ...
  if (k == 0) {
    if (cell.y < surf) {
      ...
      // Background plant skyline. Drawn first (so foreground plants
      // paint on top) with extra-wide silhouettes and atmospheric-
      ...
      if (bgChosenKind > 0) {
        ...
        if (inBgSilhouette) {
          vec3 bgTrunkCol = vec3(64.0, 44.0, 22.0) / 255.0;
      ...
      for (int dx = -5; dx <= 5; dx++) {
        ...
        // (small woody base), tree = nH / 6 — shorter than the
        // earlier nH / 4 so more canopy is visible inside the
        // typical 12-30 cell sky band.
        int trunkCells =
          (nKind == 1) ? 0 :
          ((nKind == 2) ? 1 : max(1, nH / 6));
        bool inTrunk = trunkCells > 0 && (cell.y > nBase - trunkCells);
        // Fixed kind-based radii. Heights are bounded (4/14/60) so
        // sqrt scaling barely moved values inside the clamp; constant
        // widths read more visually consistent across plants of the
        // same kind. Canopy radii are tuned so trees look bushy
        // rather than squished — width-to-height roughly tree 1:3,
        // shrub 1:1, grass tuft is a single sub-cell column.
        int trunkRadius =
          (nKind == 1) ? 0 :
          ((nKind == 2) ? 1 : 2);
        int canopyRadius =
    ...
    // Root fibers in soil. Trees (kind 3) and shrubs (kind 2) write
    // to uRoot at the cells under their plant body. Render as a
    // brown thread visible against the soil texture; the thread
    // occupies only a 1-2 sub-cell stripe down the centre of the
    // cell so the soil reads through around it.
    int rootKind = sampleU8(uRoot, cell);
    if (rootKind > 0) {
      // Thread runs vertically through the centre of the cell.
      int rootHashByte = (hashA + cell.x * 13 + cell.y * 7) & 0xff;
      int stemCenter = uSub / 2;
      int dxs = subOff.x - stemCenter;
      int reach = ((rootHashByte & 3) > 1) ? 1 : 0;
      if (dxs >= -reach && dxs <= reach) {
        vec3 rootTreeCol = vec3(60.0, 38.0, 18.0) / 255.0;
        vec3 rootShrubCol = vec3(80.0, 56.0, 28.0) / 255.0;
        vec3 rootC = (rootKind == 3) ? rootTreeCol : rootShrubCol;
        // Per-pixel hash variation so the root reads as a fiber not
        // a flat bar.
        float jitter = (float(rootHashByte & 31) / 31.0 - 0.5) * 0.18;
        col = mix(col, rootC * (1.0 + jitter), 0.85);
      }
    }
  }
